...
export class TaskList implements OnInit, OnDestroy {
  ...
  get canCreate(): boolean { return this.canWrite; }
  get canEdit(): boolean {
    return this.auth.hasRole('ADMIN', 'PMO', 'PM', 'DEV', 'QA', 'DEVOPS');
  }
  get canDelete(): boolean { return this.canWrite; }
  ...
  loadProjects(): void {
    this.projectService.getAllProjects().pipe(takeUntil(this.destroy$)).subscribe({
      next: (data) => { this.projects = Array.isArray(data) ? data : []; this.cdr.detectChanges(); },
      error: () => { this.projects = []; },
    });
  }
  ...
  loadTasks(): void {
    ...
    this.taskService.getTasksByProject(this.projectId).pipe(takeUntil(this.destroy$)).subscribe({
      next: (data) => {
      ...
      error: (err: HttpErrorResponse) => {
        ...
        this.errorMessage = err?.error?.message
          || (typeof err?.error === 'string' ? err.error : `Error ${err.status}: failed to load tasks`);
        this.cdr.detectChanges();
      },
    });
  }
  ...
  getTaskTypeBadgeClass(task: TaskDTO): string {
    switch (this.getTaskType(task)) {
      ...
      case 'subtask':   return 'badge-subtask';
      case 'milestone': return 'badge-milestone';
  ...
  get filteredTasks(): TaskDTO[] {
    ...
    result = [...result].sort((a, b) => {
      switch (this.sortBy) {
        case 'wbs':           return this.compareWbs(a.wbsNumber, b.wbsNumber);
  ...
  get totalFiltered(): number { return this.filteredTasks.length; }
  ...
  toggleActive(task: TaskDTO, event: Event): void {
    ...
    this.taskService.setTaskActive(task.id, !task.active).subscribe({
      next: () => this.loadTasks(),
      error: (err) => {
        this.errorMessage = err?.error?.message || (typeof err?.error === 'string' ? err.error : 'Error updating task status');
        this.cdr.detectChanges();
      },
    });
  }
  ...
  confirmDelete(): void {
    ...
    if (this.deleteTarget.hasChildren) {
      ...
      this.errorMessage =
        `Cannot delete '${this.deleteTarget.name}': it has active subtasks. Delete or reassign them first.`;
