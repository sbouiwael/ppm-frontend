...
import {
  ...
  HealthStatus,
} from '../../models/portfolio-dashboard';
...
export class PortfolioDashboard implements OnInit, OnDestroy {
  ...
  get filteredProjects(): ProjectHealthSummary[] {
    ...
    if (this.searchTerm.trim()) {
      ...
      result = result.filter(
        (p) =>
          p.name.toLowerCase().includes(t) || (p.portfolioName ?? '').toLowerCase().includes(t),
      );
    }
    return result;
  }
  ...
  private renderCharts(): void {
    ...
    this.mk(
      ...
      [
        {
          data: [
            d.onTrackProjectsCount,
            d.atRiskProjectsCount,
            d.delayedProjectsCount,
            d.completedProjectsCount,
          ],
    ...
    if (d.portfolioSummaries.length > 0) {
      this.mk(
        'pd-chart-portfolios',
        'bar',
        d.portfolioSummaries.map((p) =>
          p.name.length > 18 ? p.name.substring(0, 18) + '…' : p.name,
        ),
        [
          {
            label: 'Projects',
            data: d.portfolioSummaries.map((p) => p.projectCount),
            backgroundColor: PRIMARY,
          },
          {
            label: 'Avg Progress',
            data: d.portfolioSummaries.map((p) => p.avgProgress),
  ...
  private mk(id: string, type: ChartType, labels: string[], datasets: ChartDataset[]): void {
